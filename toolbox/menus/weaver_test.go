...
import (
	"cn/com/hengwei/pkg/environment/env_tests"
...
func TestMenuSimple(t *testing.T) {
	...
	for tidx, test := range tests {
		...
		weaver, err := NewWeaver(logger, env.Environment, sendEvent, map[string]Layout{
			"default": test.layout,
		})
		if err != nil {
...
var test2 = []testStep{
	{
		...
		value: testapp1,
		results: func() []toolbox.Menu {
			results := toolbox.CopyMenuArray(menuTestLayout)
